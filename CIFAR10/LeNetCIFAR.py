...
class LeNetCIFAR10(nn.Module):
    def __init__(self, num_classes: int = 10):
        super().__init__()
        # CIFAR-10 is 3x32x32
        self.conv1 = nn.Conv2d(3, 6, kernel_size=5, stride=1, padding=2)  # -> 6x32x32
        self.pool1 = nn.AvgPool2d(kernel_size=2, stride=2)                 # -> 6x16x16
        self.conv2 = nn.Conv2d(6, 16, kernel_size=5)                       # -> 16x12x12
        self.pool2 = nn.AvgPool2d(kernel_size=2, stride=2)                 # -> 16x6x6
...
def main():
    ...
    parser.add_argument('--ckpt', type=str, default='CIFAR10/lenet_cifar10_best.pth', help='path to save/load checkpoint')
    ...
    # Data
    train_loader, test_loader = get_loaders(batch_size=cfg.batch_size, augment=cfg.augment)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        scheduler.step()
        test_acc = accuracy(model, test_loader, device)
        print(f'Epoch {epoch:02d}/{cfg.epochs} | Loss: {train_loss:.4f} | Test Acc: {test_acc:.2f}% | LR: {scheduler.get_last_lr()[0]:.5f}')
        ...
        if test_acc > best_acc:
            ...
            torch.save({'model': model.state_dict(),
                        'acc': best_acc,
                        'epoch': epoch}, cfg.ckpt_path)
            print(f'  -> Saved new best to {cfg.ckpt_path} (acc={best_acc:.2f}%)')
