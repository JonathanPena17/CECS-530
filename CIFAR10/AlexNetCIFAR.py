...
def cifar_alexnet(num_classes=10):
    m = alexnet(pretrained=False)
    # Make CIFAR-friendly: smaller first conv and remove initial maxpool
    m.features[0] = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
    # Remove first maxpool (index 2 in torchvision alexnet features)
    m.features[2] = nn.Identity()
    # Keep AdaptiveAvgPool2d((6,6)) as is to stabilize classifier input size
...
def main():
    ...
    parser.add_argument('--ckpt', type=str, default='alexnet_cifar10_best.pth')
